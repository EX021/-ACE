...
ITEMS_DATA = {
    ...
    "katana_technologique": {
        "name": "Katana technologique",
        "type": "weapon",
        "power": 1000000000,
    },
    # ══════════════════════════════════════
    # ⚔️  NOUVEAUX ITEMS MAUDITS
    # ══════════════════════════════════════
    "lame_parasite": {
        "name": "Lame parasite",
        "type": "weapon",
        "power": 300,
        "cursed": True,
        "parasite": True,          # inflige 100% des dégâts au joueur aussi
        "niveau": 1,
        "evolution": None,         # None / "maitrisee" / "dechainees"
        "rarity": "Légendaire",
        "lore": "Elle ne tranche pas… elle échange. Et elle ne fait jamais de cadeau.",
    },
    "totem_immortalite": {
        "name": "Totem d'immortalité",
        "type": "totem",
        "actif": True,             # False = épuisé, besoin de recharge
        "evolution": None,         # None / "stabilise" / "corrompu"
        "rarity": "Légendaire",
        "lore": "Ce totem ne te sauve pas… il te laisse simplement mourir plus tard.",
    },
    "armure_neant": {
        "name": "Armure du Néant",
        "type": "armor",
        "actif": False,            # True = bouclier actif ce tour
        "charges": 0,              # nombre de tours d'invulnérabilité restants
        "evolution": None,         # None / "stabilisee" / "corrompue"
        "rarity": "Légendaire",
        "lore": "Cette armure ne bloque pas les attaques… elle les efface de la réalité.",
    },
    "fiole_occulte": {
        "name": "Fiole d'énergie occulte",
        "type": "resource_speciale",
        "rarity": "Rare",
        "lore": "Une simple fiole suffit à défier la mort… mais personne ne sait ce qu'elle consume en échange.",
    },
    # ══════════════════════════════════════
    # Consommables / Outils
    # ══════════════════════════════════════
    ...
    "cle_usb_defence_lvl1": {
        "name": "Clé USB #PROGRAMME DE DÉFENSE LVL1#",
        "type": "tool",
    },
    "carte_acces_lvl1": {
...
AMELIORATIONS_DATA = {
    ...
    "lame_parasite": {
        "niveau_max": 100,
        "cout_or_par_niveau": 800,
        "bonus_power_par_niveau": 120,
        # À niveau 100 : évolution disponible (gérée séparément)
    },
}
...
joueur = {
    ...
    "degats_base": 10,
    # ── État des artefacts ─────────────────
    "totem_actif": False,          # True si le totem est rechargé et prêt
    "armure_charges": 0,           # Tours d'invulnérabilité restants
    "armure_post_malus": False,    # True si l'Armure corrompue a déjà été utilisée ce combat
}
...
# ==========================================
# FONCTIONS ARTEFACTS MAUDITS
# ==========================================

def afficher_statut_artefacts():
    """Affiche l'état des artefacts maudits dans l'inventaire."""
    lignes = []
    if a_item("totem_immortalite"):
        etat = "✅ ACTIF" if joueur["totem_actif"] else "💤 Épuisé (besoin d'1 fiole)"
        lignes.append(f"  💀 Totem d'immortalité : {etat}")
    if a_item("armure_neant"):
        charges = joueur["armure_charges"]
        if charges > 0:
            lignes.append(f"  🛡️  Armure du Néant : ⚡ ACTIVE ({charges} tour(s) restant(s))")
        else:
            fioles = get_qty("fiole_occulte")
            cout = _cout_armure()
            lignes.append(f"  🛡️  Armure du Néant : Inactive — coût activation : {cout} fioles (tu en as {fioles})")
    if a_item("lame_parasite"):
        item = joueur["inventaire"]["lame_parasite"]
        evo = item.get("evolution") or "Aucune"
        lignes.append(f"  🩸 Lame parasite : power {item.get('power', 300)} | Évolution : {evo}")
    if lignes:
        print("\n🔮 Artefacts maudits :")
        for l in lignes:
            print(l)
...
def _cout_armure():
    """Retourne le coût en fioles de l'Armure selon son évolution."""
    if not a_item("armure_neant"):
        return 5
    evo = joueur["inventaire"]["armure_neant"].get("evolution")
    if evo == "stabilisee":
        return 3
    return 5
...
def _duree_armure():
    """Retourne la durée en tours de l'Armure selon son évolution."""
    if not a_item("armure_neant"):
        return 1
    evo = joueur["inventaire"]["armure_neant"].get("evolution")
    if evo == "corrompue":
...
def activer_armure_neant():
    """Active l'Armure du Néant si le joueur a assez de fioles."""
    cout = _cout_armure()
    duree = _duree_armure()
    fioles = get_qty("fiole_occulte")

    if fioles < cout:
        print(f"  ❌ Pas assez de fioles ! (il faut {cout}, tu en as {fioles})")
        return False

    retirer_item("fiole_occulte", cout)
    joueur["armure_charges"] = duree
    evo = joueur["inventaire"]["armure_neant"].get("evolution")
    if evo == "corrompue":
        joueur["armure_post_malus"] = True   # on notera d'appliquer le malus après
    print(f"  🛡️  ARMURE DU NÉANT activée ! Invulnérabilité pendant {duree} tour(s) !")
    time.sleep(1)
    return True


def recharger_totem():
    """Recharge le Totem d'immortalité avec 1 fiole occulte."""
    if not a_item("totem_immortalite"):
        print("  Tu ne possèdes pas le Totem d'immortalité.")
        return False
    if joueur["totem_actif"]:
        print("  Le totem est déjà actif.")
        return False
    if not a_item("fiole_occulte"):
        print("  ❌ Il te faut 1 Fiole d'énergie occulte pour recharger le totem.")
        return False
    retirer_item("fiole_occulte")
    joueur["totem_actif"] = True
    print("  💀 Totem d'immortalité rechargé ! Tu peux défier la mort une fois.")
    time.sleep(1)
    return True


def evoluer_item_maudit(item_id):
    """Propose l'évolution d'un item maudit au niveau 100."""
    item = joueur["inventaire"].get(item_id)
    if not item:
        return

    if item_id == "lame_parasite":
        if item.get("niveau", 1) < 100:
            print("  La Lame parasite doit atteindre le niveau 100 pour évoluer.")
            return
        if item.get("evolution"):
            print(f"  La Lame parasite a déjà évolué : {item['evolution']}")
            return
        print("\n⚖️  ÉVOLUTION — Lame parasite niveau 100")
        print("  🛡️  Lame maîtrisée : dégâts subis réduits à 70% | style défensif")
        print("  💀 Lame déchaînée : dégâts x2 MAIS tu subis 150% | style offensif ultra-risqué")
        choix = questionary.select(
            "Quelle voie choisir ?",
            choices=[
                questionary.Choice("🛡️  Lame maîtrisée", value="maitrisee"),
                questionary.Choice("💀 Lame déchaînée", value="dechainees"),
                questionary.Choice("❌ Plus tard", value=None),
            ]
        ).ask()
        if choix:
            item["evolution"] = choix
            print(f"  ✅ Lame parasite → {choix} !")
            time.sleep(1)

    elif item_id == "totem_immortalite":
        if item.get("evolution"):
            print(f"  Le Totem a déjà évolué : {item['evolution']}")
            return
        print("\n⚖️  ÉVOLUTION — Totem d'immortalité")
        print("  🛡️  Totem stabilisé : recharge fiable, aucun effet secondaire")
        print("  💀 Totem corrompu  : après résurrection, +50% dégâts temporaire MAIS perte de 15 PV/tour pendant 3 tours")
        choix = questionary.select(
            "Quelle voie choisir ?",
            choices=[
                questionary.Choice("🛡️  Totem stabilisé", value="stabilise"),
                questionary.Choice("💀 Totem corrompu", value="corrompu"),
                questionary.Choice("❌ Plus tard", value=None),
            ]
        ).ask()
        if choix:
            item["evolution"] = choix
            print(f"  ✅ Totem d'immortalité → {choix} !")
            time.sleep(1)

    elif item_id == "armure_neant":
        if item.get("evolution"):
            print(f"  L'Armure a déjà évolué : {item['evolution']}")
            return
        print("\n⚖️  ÉVOLUTION — Armure du Néant")
        print("  🛡️  Armure stabilisée : coût réduit à 3 fioles, 1 tour, aucun effet secondaire")
        print("  💀 Armure corrompue  : 2 tours d'invulnérabilité MAIS -30 PV et -20% dégâts pendant 2 tours après")
        choix = questionary.select(
            "Quelle voie choisir ?",
            choices=[
                questionary.Choice("🛡️  Armure stabilisée", value="stabilisee"),
                questionary.Choice("💀 Armure corrompue", value="corrompue"),
                questionary.Choice("❌ Plus tard", value=None),
            ]
        ).ask()
        if choix:
            item["evolution"] = choix
            print(f"  ✅ Armure du Néant → {choix} !")
            time.sleep(1)
...
def afficher_barres_combat(pv_ennemi, pv_ennemi_max, nom_ennemi):
    ...
    # Indicateurs d'artefacts actifs
    if joueur["armure_charges"] > 0:
        print(f"  🛡️  ARMURE DU NÉANT ACTIVE — {joueur['armure_charges']} tour(s)")
    if joueur["totem_actif"]:
        print(f"  💀 Totem d'immortalité : PRÊT")
    print()
        ...
        choices=[
            questionary.Choice(
                title=f"{joueur['inventaire'][w]['name']} (puissance: {joueur['inventaire'][w].get('power', '?')})"
                      + (" 🩸MAUDITE" if joueur['inventaire'][w].get('cursed') else ""),
                value=w
            )
            for w in weapons
...
def combat(ennemi_id):
    ...
    Intègre :
      - Lame parasite (symbiose : dégâts infligés = dégâts subis)
      - Totem d'immortalité (survie à 1 PV au lieu de mourir)
      - Armure du Néant (1 ou 2 tours d'invulnérabilité totale)
    """
    ...
    # Réinitialisation des états de combat
    joueur["armure_post_malus"] = False
    malus_armure_corrompue_tours = 0   # nombre de tours de malus restants après armure corrompue
    ...
    # Avertissement lame maudite
    if arme_id == "lame_parasite":
        print("  🩸 ATTENTION : La Lame parasite te fera subir les mêmes dégâts qu'elle inflige !")
        time.sleep(1.5)
    ...
    while joueur["pv"] > 0 and pv_ennemi > 0:
        ...
        # ── Construire les options d'action ───────────────────────────────
        options = [
        ...
        # Option Armure du Néant (si possédée, inactive, et assez de fioles)
        if a_item("armure_neant") and joueur["armure_charges"] == 0:
            cout = _cout_armure()
            fioles = get_qty("fiole_occulte")
            options.append(questionary.Choice(
                title=f"🛡️  Activer l'Armure du Néant  ({cout} fioles — tu en as {fioles})",
                value="armure"
            ))

        # Option recharger Totem
        if a_item("totem_immortalite") and not joueur["totem_actif"] and a_item("fiole_occulte"):
            options.append(questionary.Choice(
                title=f"💀 Recharger le Totem d'immortalité  (1 fiole)",
                value="totem_recharge"
            ))

        action = questionary.select("Que fais-tu ?", choices=options).ask()
        ...
        if action == "attaquer":
            ...
            # Malus Armure corrompue : -20% dégâts pendant les tours de malus
            if malus_armure_corrompue_tours > 0:
                degats_tour = max(1, int(degats_tour * 0.8))
                print(f"  ⚠️  Malus Armure corrompue : dégâts réduits !")

            pv_ennemi -= degats_tour
            print(f"  ⚔️  Tu frappes {nom} pour {degats_tour} dégâts !")
            time.sleep(0.6)

            # ── LAME PARASITE : symbiose 100% ─────────────────────────────
            if arme_id == "lame_parasite":
                item_lame = joueur["inventaire"].get("lame_parasite", {})
                evo = item_lame.get("evolution")

                if evo == "dechainees":
                    # Dégâts x2, retour 150%
                    bonus = degats_tour  # déjà x2 via power, le retour est 150%
                    retour = max(1, int(degats_tour * 1.5))
                    print(f"  💀 LAME DÉCHAÎNÉE : dégâts doublés, mais elle te consume pour {retour} PV !")
                elif evo == "maitrisee":
                    retour = max(1, int(degats_tour * 0.7))
                    print(f"  🩸 Lame maîtrisée : retour réduit à {retour} PV subis.")
                else:
                    # Version de base : 100% retour
                    retour = degats_tour
                    print(f"  🩸 La lame échange — tu subis aussi {retour} dégâts !")

                # L'armure bloque le retour de la lame
                if joueur["armure_charges"] > 0:
                    print(f"  🛡️  L'Armure du Néant absorbe le retour de la lame !")
                else:
                    joueur["pv"] -= retour
            ...
            activer_armure_neant()

        elif action == "totem_recharge":
            recharger_totem()

        # ── TOTEM CORROMPU : malus actif post-résurrection ────────────────
        # (géré dans la section mort plus bas)

        # ── RIPOSTE DE L'ENNEMI ───────────────────────────────────────────
        if pv_ennemi > 0:
            # Vérifier armure AVANT la riposte
            if joueur["armure_charges"] > 0:
                print(f"  🛡️  {nom} attaque — mais l'Armure du Néant annule tout !")
                joueur["armure_charges"] -= 1
                if joueur["armure_charges"] == 0:
                    print("  ⚡ L'effet de l'Armure du Néant se dissipe.")
                    # Appliquer le malus de l'armure corrompue
                    if joueur["armure_post_malus"]:
                        joueur["pv"] = max(1, joueur["pv"] - 30)
                        malus_armure_corrompue_tours = 2
                        joueur["armure_post_malus"] = False
                        print(f"  ☠️  MALUS Armure corrompue : -30 PV et -20% dégâts pendant 2 tours !")
                time.sleep(0.8)
        ...
        # Décrémenter malus armure corrompue
        if malus_armure_corrompue_tours > 0:
            malus_armure_corrompue_tours -= 1

        # ── VÉRIFICATION MORT DU JOUEUR ───────────────────────────────────
        if joueur["pv"] <= 0:
            # Totem d'immortalité : dernière chance
            if joueur["totem_actif"]:
                joueur["pv"] = 1
                joueur["totem_actif"] = False
                print(f"\n  💀 Tu tombes... mais le Totem d'immortalité refuse ta mort !")
                print(f"  Tu te relèves avec 1 PV !")

                # Totem corrompu : bonus dégâts + malus
                if a_item("totem_immortalite"):
                    evo_totem = joueur["inventaire"]["totem_immortalite"].get("evolution")
                    if evo_totem == "corrompu":
                        # Bonus dégâts temporaire via un flag (simplifié : boost direct ce tour)
                        print(f"  ☠️  TOTEM CORROMPU : tu gagnes +50% dégâts temporaires...")
                        print(f"  ☠️  ... mais tu perdras 15 PV par tour pendant 3 tours !")
                        # On stocke ce malus temporairement dans joueur
                        joueur["totem_malus_tours"] = joueur.get("totem_malus_tours", 0) + 3
                time.sleep(2)
            else:
                # Mort définitive
        ...
        # Malus totem corrompu : perte de PV progressive
        if joueur.get("totem_malus_tours", 0) > 0:
            joueur["pv"] = max(1, joueur["pv"] - 15)
            joueur["totem_malus_tours"] -= 1
            print(f"  ☠️  Malus Totem corrompu : -15 PV ! ({joueur['totem_malus_tours']} tours restants)")
            if joueur["pv"] <= 1 and not joueur["totem_actif"]:
                print("  ⚠️  Tu es à l'agonie ! Utilise un soin rapidement !")
    ...
    if pv_ennemi <= 0:
        ...
        # Réinitialiser le malus totem après combat
        joueur["totem_malus_tours"] = 0
...
def menu_coffre():
    while True:
        ...
        if choix == "❌ Fermer":
        ...
        elif choix == "👁️  Voir le coffre":
            if coffre:
                print("\n🗃️ Contenu du coffre :")
                for iid, it in coffre.items():
                    print(f"  - {it['name']} x{it['qty']}")
            else:
                print("  Le coffre est vide.")
            time.sleep(1.5)

        elif choix == "📥 Déposer un item":
            items_inv = list(joueur["inventaire"].keys())
            if not items_inv:
            ...
            item_choisi = questionary.select(
                ...
                choices=[
                    questionary.Choice(
                        title=f"{joueur['inventaire'][i]['name']} x{joueur['inventaire'][i]['qty']}",
                        value=i
                    )
                    for i in items_inv
                ] + [questionary.Choice(title="❌ Annuler", value=None)]
            ...
            if item_choisi:
                qty = joueur["inventaire"][item_choisi]["qty"]
                retirer_item(item_choisi, qty)
                if item_choisi in coffre:
                    coffre[item_choisi]["qty"] += qty
                else:
                    data = get_item_data(item_choisi).copy()
                    data["qty"] = qty
                    coffre[item_choisi] = data
                print(f"✅ Déposé dans le coffre.")
                time.sleep(1)

        elif choix == "📤 Récupérer un item":
            if not coffre:
                print("Le coffre est vide.")
                time.sleep(1)
                continue
            item_choisi = questionary.select(
                ...
                choices=[
                    questionary.Choice(
                        title=f"{coffre[i]['name']} x{coffre[i]['qty']}",
                        value=i
                    )
                    for i in coffre
                ] + [questionary.Choice(title="❌ Annuler", value=None)]
            ...
            if item_choisi:
                qty = coffre[item_choisi]["qty"]
                ajouter_item(item_choisi, qty)
                del coffre[item_choisi]
                print(f"✅ Récupéré depuis le coffre.")
                time.sleep(1)
...
def ameliorer_item(item_id):
    ...
    if niveau_actuel >= niveau_max:
        ...
        # Proposer l'évolution si item maudit
        if item_id in ("lame_parasite", "totem_immortalite", "armure_neant"):
            evoluer_item_maudit(item_id)
        return
    ...
    # Évolution auto au niveau 100
    if item["niveau"] >= niveau_max and item_id in ("lame_parasite",):
        print(f"\n🔮 {item['name']} a atteint le niveau maximum ! Une évolution est disponible.")
        evoluer_item_maudit(item_id)
...
def ouvrir_menu():
    ...
    while True:
        ...
        if choix == "📦 Voir l'inventaire":
            ...
            afficher_statut_artefacts()
            time.sleep(2)
        elif choix == "🗃️ Coffre de stockage":
            menu_coffre()
        ...
        elif choix == "🔮 Artefacts maudits":
            items_maudits = ["lame_parasite", "totem_immortalite", "armure_neant"]
            possédes = [i for i in items_maudits if a_item(i)]
            if not possédes:
                print("\n🔮 Tu ne possèdes aucun artefact maudit.")
                time.sleep(1)
                continue
            ...
            item_choisi = questionary.select(
                ...
                choices=[
                    questionary.Choice(
                        title=joueur["inventaire"][i]["name"],
                        value=i
                    ) for i in possédes
            ...
            choix_action_art = [
                questionary.Choice("📖 Voir le lore", value="lore"),
                questionary.Choice("⚖️  Évoluer (si niveau max)", value="evoluer"),
            ]
            if item_choisi == "totem_immortalite":
                choix_action_art.append(questionary.Choice("🔋 Recharger le Totem (1 fiole)", value="totem"))
            choix_action_art.append(questionary.Choice("❌ Retour", value=None))

            action_art = questionary.select(
                "Que veux-tu faire ?",
                choices=choix_action_art
            ).ask()

            if action_art == "lore":
                lore = ITEMS_DATA.get(item_choisi, {}).get("lore", "Aucun lore disponible.")
                print(f'\n  🎬 "{lore}"')
                time.sleep(2)
            elif action_art == "evoluer":
                evoluer_item_maudit(item_choisi)
            elif action_art == "totem":
                recharger_totem()
            ...
            if CODE == "PLUS ULTRA":
                ...
                print("Récompense accordée !")
            elif CODE == "LEGO":
                ajouter_item("katana_technologique")
                print("Récompense accordée !")
            elif CODE == "MEDIC":
                ajouter_item("kit_soin", 5)
                print("Récompense accordée !")
            elif CODE == "OCCULT":
                ajouter_item("fiole_occulte", 3)
                print("3 Fioles d'énergie occulte accordées !")
            elif CODE == "PARASITE":
                ajouter_item("lame_parasite")
                print("Récompense accordée — attention, elle mord.")
            elif CODE == "IMMORTEL":
                ajouter_item("totem_immortalite")
                joueur["totem_actif"] = False
                print("Récompense accordée — il faudra une fiole pour l'activer.")
            elif CODE == "NEANT":
                ajouter_item("armure_neant")
                print("Récompense accordée — le vide t'appartient.")
            else:
            ...
            choix_boutique = questionary.select(
                ...
                choices=[
                    ...
                    "Batterie (70 OR)",
                    "1 Boulon (1 OR)",
                    "Canon à antimatière (1 000 000 000 OR)",
            ...
            if choix_boutique == "Pistolet (55 OR)":
                if or_actuel >= 55:
                    ...
                    print("Transaction réussie  [-55 OR]")
                else:
                ...
                if or_actuel >= 30:
                    ...
                    print("Transaction réussie  [-30 OR]")
                else:
                ...
                if or_actuel >= 70:
                    ...
                    print("Transaction réussie  [-70 OR]")
                else:
                ...
                if or_actuel >= 1:
                    retirer_item("or", 1)
                    ajouter_item("boulon")
                    print("Transaction réussie  [-1 OR]")
                else:
                    print("Pas assez d'or !")

            elif choix_boutique == "Canon à antimatière (1 000 000 000 OR)":
                if or_actuel >= 1000000000:
                    retirer_item("or", 1000000000)
                    ajouter_item("canon_antimatiere")
                    print("Transaction réussie  [-1 000 000 000 OR]")
                else:
        ...
        elif choix == "🔴 Mises à jour":
            print("╔══════🔴 Mises à Jour Récentes 🔴══════╗")
            print("✅ Correction des noms d'items (clés unifiées)")
            print("✅ Correction orthographe (scientifique, défense...)")
            print("✅ Nouveaux items : Lame parasite, Totem d'immortalité")
            print("✅ Nouveaux items : Armure du Néant, Fiole d'énergie occulte")
            print("✅ Mécaniques : symbiose Lame, résurrection Totem")
            print("✅ Mécaniques : invulnérabilité Armure du Néant")
            print("✅ Évolutions à niveau 100 pour les items maudits")
            print("✅ Onglet Artefacts maudits dans le menu")
            print("✅ Codes secrets pour les nouveaux items")
            print("✅ Équilibrage général du combat")
            print("✅ Amélioration du système combat (affichage artefacts)")
            print("⚙️  Étape 5 (en cours)")
            time.sleep(3)
            ...
            if not dev_mode:
                ...
                choix_dev = questionary.select(
                    ...
                    choices=[
                        ...
                        "❤️  Remplir les PV",
                        "⭐ Ajouter de l'XP",
                        "🧪 Ajouter des fioles occultes",
                        "💀 God Mode (PV infinis)",
                        "🔮 Donner tous les artefacts maudits",
                ...
                if choix_dev == "🔢 Changer d'étape":
                    ...
                    if item_id in ITEMS_DATA:
                        ...
                        # Init totem si ajouté
                        if item_id == "totem_immortalite":
                            joueur["totem_actif"] = False
                    else:
                        print("❌ Item inconnu.")
                    time.sleep(1)
                ...
                elif choix_dev == "🧪 Ajouter des fioles occultes":
                    nb = input("Nombre de fioles (défaut 5) : ").strip()
                    nb = int(nb) if nb.isdigit() else 5
                    ajouter_item("fiole_occulte", nb)
                    time.sleep(1)
                ...
                elif choix_dev == "🔮 Donner tous les artefacts maudits":
                    ajouter_item("lame_parasite")
                    ajouter_item("totem_immortalite")
                    ajouter_item("armure_neant")
                    ajouter_item("fiole_occulte", 10)
                    joueur["totem_actif"] = False
                    print("✅ Tous les artefacts maudits ajoutés + 10 fioles !")
                    time.sleep(1)
...
if os.path.exists(FICHIER_SAVE):
    ...
    if reprendre:
        ...
        joueur.setdefault("totem_actif", False)
        joueur.setdefault("armure_charges", 0)
        joueur.setdefault("armure_post_malus", False)
        joueur.setdefault("inventaire", {})
        joueur.pop("donjon_etage_max", None)

        # Migration ancienne sauvegarde (clés obsolètes)
        migration_clés = {
            "Serigue explosive": "seringue_explosive",
            "Canon a antimatière": "canon_antimatiere",
            "Katana technologique": "katana_technologique",
            "Seringue soignante": "seringue_soignante",
        }
        for ancienne, nouvelle in migration_clés.items():
            if ancienne in joueur["inventaire"]:
                val = joueur["inventaire"].pop(ancienne)
                if nouvelle not in joueur["inventaire"]:
                    joueur["inventaire"][nouvelle] = val
                else:
                    joueur["inventaire"][nouvelle]["qty"] += val.get("qty", 1)
...
def etape_4():
    ...
    print("En arrivant tu trouves par terre une clé USB avec marqué #PROGRAMME DE DÉFENSE LVL1#")
    print("Tu la prends.")
    ...
    if choix5 == "OUI":
        ...
        ajouter_item("seringue_explosive")
        ajouter_item("kit_soin", 2)
        ajouter_item("seringue_soignante", 3)

        # ── COFFRE SECRET : Artefacts maudits ──────────────────────────────
        print("\nDans un coin de la salle tu aperçois un coffre scellé...")
        time.sleep(2)
        print("Il s'ouvre à ton approche, comme s'il t'attendait.")
        time.sleep(2)
        print("À l'intérieur : une lame noire qui pulse, un totem de bois sombre, et 3 fioles étranges.")
        time.sleep(2)
        ajouter_item("lame_parasite")
        ajouter_item("totem_immortalite")
        ajouter_item("fiole_occulte", 3)
        joueur["totem_actif"] = False
        print("\n  🎬 \"Elle ne tranche pas… elle échange. Et elle ne fait jamais de cadeau.\"")
        time.sleep(3)
